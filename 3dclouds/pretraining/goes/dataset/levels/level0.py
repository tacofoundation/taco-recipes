...
def _datetime_to_microseconds(dt_str: str) -> int:
    """Convert ISO datetime string to microseconds since Unix epoch (UTC)."""
    return int(isoparse(dt_str).timestamp() * 1_000_000)


# Sample builders - one function per file type
def build_sample_goes(ctx: dict) -> Sample:
    """
    GOES GeoTIFF sample with STAC metadata.
    ...
    # Extract timestamps - convert to microseconds
    time_start = _datetime_to_microseconds(tags["time_coverage_start"])
    time_end = _datetime_to_microseconds(tags["time_coverage_end"])
    ...
    sample.extend_with(Satellite(satellite="GOES"))
