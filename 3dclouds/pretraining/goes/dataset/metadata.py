...
This module loads GOES GeoTIFF files and provides them as contexts.
...
def load_contexts(limit: float | int | None = None) -> list[dict]:
    """
    Load GOES GeoTIFF files and return list of context dicts.

    Scans two directories:
    - /data/databases/CLOUD_3D/pretraining/geotiff/goes/
    - /data/databases/CLOUD_3D/pretraining/geotiff/cyclones/goes/
    ...
    goes_path = root / "goes"
    cyclones_path = root / "cyclones" / "goes"
    
    tif_files = []
    
    # Add regular GOES files
    if goes_path.exists():
        tif_files.extend(list(goes_path.rglob("*.tif")))
    
    # Add cyclone GOES files
