...
from dataset.extensions import Satellite, Cyclone
...
def build_sample_himawari(ctx: dict) -> Sample:
    """
    HIMAWARI GeoTIFF sample with STAC metadata.
    
    Reads existing .tif file and extracts:
    - STAC metadata with corrected EPSG:4326 centroid
    - TacoTIFF Header
    - GeotiffStats
    - cloud3d custom metadata
    """
    ...
    # Extract timestamp from tags (HIMAWARI uses acquisition_time)
    # Convert to microseconds (STAC expects int64 μs since Unix epoch)
    acquisition_time_us = int(isoparse(tags["acquisition_time"]).timestamp() * 1_000_000)
    
    # Add STAC metadata with corrected centroid
    # check_antimeridian=True because HIMAWARI covers the Pacific and crosses ±180°
    stac = STAC(
    ...
    sample.extend_with(Satellite(satellite="HIMAWARI"))
    sample.extend_with(Cyclone(is_cyclone=ctx["is_cyclone"]))
    
    return sample


SAMPLES = [
    build_sample_himawari,
]
...
def _build_samples_parallel(ctx: dict) -> tuple[list[Sample] | None, tuple[str, str] | None]:
    ...
    try:
        ...
        # Validate geometry (no infinites, valid shapes, not empty)
        for sample in samples:
            geom = shapely.from_wkb(sample.model_dump()["stac:centroid"])
            if geom.is_empty:
                return None, (ctx["id"], "Empty centroid geometry (antimeridian issue?)")
...
#
#ctx = {
#    'id': '20190813213000_4246_2553',
#    'path': pathlib.Path('/data/databases/CLOUD_3D/pretraining/geotiff/himawari/l1b/20190813213000_4246_2553.tif'),
#    'is_cyclone': True
#}
